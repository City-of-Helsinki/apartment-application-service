...
def test_application_model():
    ...
    ApplicationFactory(id=application_uuid)

    assert Application.objects.first().id == application_uuid
...
def test_application_has_apartments_model():
    ...
    apartments = ApartmentFactory.create_batch_with_project(5, project=ProjectFactory())
    ...
    assert str(list(apartments_application)[0].id) == application.id
    # check that firs occurance of ApartmenApplication object application's id is same
    # as firstly created application
    assert str(ApplicationApartment.objects.first().application.id) == application.id
