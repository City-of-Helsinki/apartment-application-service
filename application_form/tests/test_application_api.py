...
@pytest.mark.django_db
def test_get_apartment_states_reserved_after_sale_canceled_and_new_offer(
    drupal_server_api_client,
    elastic_haso_project_with_5_apartments,
):
    """
    When a SOLD reservation is canceled and the next in queue gets an offer,
    the apartment should show as RESERVED (not SOLD) since no reservation
    is SOLD anymore.
    """
    haso_project_uuid, haso_apartments = elastic_haso_project_with_5_apartments
    apartment = haso_apartments[0]
    ...
    res1.set_state(ApartmentReservationState.SOLD)
    res1.set_state(ApartmentReservationState.CANCELED)
    res2.set_state(ApartmentReservationState.OFFERED)
