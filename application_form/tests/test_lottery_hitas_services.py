...
def test_lottery_for_small_apartment_does_not_prioritize_applications_with_children():
    ...
    # The application state also should have changed
    assert single_app.queue_application.state == ApartmentReservationState.RESERVED
    # The loser's state should have stayed the same
    assert family_app.queue_application.state == ApartmentReservationState.SUBMITTED
...
def test_lottery_for_large_apartment_prioritizes_applications_with_children():
    ...
    # The application state also should have changed
    assert family_app.queue_application.state == ApartmentReservationState.RESERVED
    # The loser's state should have stayed the same
    assert single_app.queue_application.state == ApartmentReservationState.SUBMITTED
...
def test_canceling_winning_application_marks_next_application_in_queue_as_reserved():
    ...
    assert family_app.queue_application.state == ApartmentReservationState.CANCELED
    assert single_app.queue_application.state == ApartmentReservationState.RESERVED
...
def test_becoming_first_after_lottery_does_not_cancel_low_priority_reservation():
    ...
    assert family_big.queue_application.state == ApartmentReservationState.RESERVED

    # The single applicant's low priority application should be "RESERVED"
    assert list(get_ordered_applications(tiny)) == [single]
    assert single_tiny.queue_application.state == ApartmentReservationState.RESERVED
    ...
    single_big.refresh_from_db()
    assert single_big.queue_application.state == ApartmentReservationState.RESERVED
...
def test_winning_high_priority_apartment_redistributes_apartments_if_winner_changed():
    ...
    assert app1_apt1.queue_application.state == ApartmentReservationState.RESERVED
    assert app3_apt2.queue_application.state == ApartmentReservationState.RESERVED
    assert app2_apt3.queue_application.state == ApartmentReservationState.RESERVED

    # All the canceled applications should have been marked as "CANCELED"
    assert app1_apt2.queue_application.state == ApartmentReservationState.CANCELED
    assert app1_apt3.queue_application.state == ApartmentReservationState.CANCELED
    assert app2_apt1.queue_application.state == ApartmentReservationState.CANCELED
    assert app2_apt2.queue_application.state == ApartmentReservationState.CANCELED
    assert app3_apt3.queue_application.state == ApartmentReservationState.CANCELED
...
def test_winning_low_priority_apartment_does_not_cancel_higher_priority_applications():
    ...
    assert list(get_ordered_applications(tiny)) == [single]
    assert single_low.queue_application.state == ApartmentReservationState.RESERVED

    # The other application should still be in the queue, since it was higher priority
    assert single_high.queue_application.state == ApartmentReservationState.SUBMITTED
